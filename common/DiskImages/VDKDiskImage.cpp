...
bool CVDKDiskImage::Save( const std::string& _filename )
{
	...
	if( vdkHead.header_len > (sizeof(vdkHead) + uNameLen) )
	{
...
unsigned int CVDKDiskImage::New( unsigned char uTracks, unsigned char uSides, unsigned char uSecsPerTrack, unsigned int uSectorSize )
{
	// VDK files have always 256 bytes sectors, so any attempt to use any other sector size is invalid.
	if( uSectorSize != VDK_SECTORSIZE )
	{
		return 0;
	}
...
size_t CVDKDiskImage::GetSectorSize( unsigned int _track, unsigned int _side, unsigned int _sector )
{
	// VDK files always have 256-byte sectors.
	return VDK_SECTORSIZE;
}

size_t CVDKDiskImage::GetSectorSize()
{
	// VDK files always have 256-byte sectors.
	return VDK_SECTORSIZE;
}

size_t CVDKDiskImage::GetDataSize()
{
	return dataBlockSize;
}
