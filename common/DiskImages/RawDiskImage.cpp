...
unsigned int CRAWDiskImage::New( unsigned char uTracks, unsigned char uSides, unsigned char uSecsPerTrack, unsigned int uSectorSize )
{
	mSidesNum   = uSides;
	mTracksNum  = uTracks;
	mSectorsNum = uSecsPerTrack;
	mSectorSize = uSectorSize;
	mDataSize   = mTracksNum * mSidesNum * mSectorsNum * mSectorSize;

	if(nullptr != mDataBlock)
	{
		delete mDataBlock;
	}

	mDataBlock = new unsigned char[mDataSize];
	if( nullptr == mDataBlock )
	{
		mSidesNum   = 0;
		mTracksNum  = 0;
...
std::string CRAWDiskImage::GetDiskInfo()
{
	...
	sstream << "Total size  : "   << mDataSize << " bytes/" << mDataSize / 1024 << " KB" << std::endl;
	retVal = sstream.str();
...
unsigned int CRAWDiskImage::GetSectorID( unsigned int uTrack, unsigned int uSide, unsigned int uSector ) const
{
	return uSector+1;
}